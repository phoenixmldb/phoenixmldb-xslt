        ...
        XqtsTestCase testCase, XqtsAssertion assertion, object? result, CancellationToken ct)
    {
        switch (assertion.Type)
        {
            ...
            case "assert-type":
                ...
                // XQuery type system with .NET type tests. VerifyType knew 25 atomic names and ended
                // `_ => true // Unknown type, assume pass`, so 520 of the corpus's 1,033 assert-type
                // assertions — array(*) 121, map(*) 89, xs:dateTime 38 — passed whatever the result
                // was. It also judged by .NET type, so xs:anyURI, xs:NCName and xs:language all meant
                // `result is string`: wrong in BOTH directions.
                //
                // NOT engine-first-with-legacy-fallback. That shape is monotonic where the fallback
                // only rescues; here the legacy rescue IS the defect.
                //
                // This repo carries its own copy of this runner, 217 lines drifted from the one in
                // phoenixmldb-xquery, and BOTH had this same fail-open while publishing DIFFERENT QT3
                // figures. Fixing one would have made two disagreeing numbers disagree for a new
                // reason, so the change is applied to both.
                return await VerifyXPathAssertAsync(
