...
Reported by Martin Honnen. The function returned the empty sequence unconditionally — no output,
no error — and the source said why:
...
The tell was in the registration: every other context-dependent XSLT function is constructed with
the execution context, and this one was not, so it structurally could not know the answer.

Worth being precise about what was and was not broken. Writing to stdout, the empty sequence is
CORRECT — XSLT 3.0 §20.3.7 says the property is absent when no destination URI is known. It was
wrong the moment a destination existed: `xslt … -o result.xml` names one, and an
`xsl:result-document/@href` names another.

The host now supplies the base output URI (`XsltTransformer.SetBaseOutputUri`, set by the `xslt`
tool from `-o`), `xsl:result-document` reports its own destination for the duration of its body
with a relative `@href` resolved against that base, and the previous value is restored afterwards
so a following instruction still sees the principal destination. Absent a host-supplied URI the
behaviour is unchanged, because there the empty sequence is the right answer.

### A missing required attribute crashed instead of being diagnosed

`<xsl:key match="p" use=".//term"/>` — no `name` — produced
`Object reference not set to an instance of an object.` The parser reached for the attribute with
the null-forgiving `!`, which asserts a required attribute is present: an assertion that only
holds when there is nothing to report.

The same shape appeared 25 times in the parser. Searching the conformance logs for the runtime
message found eight reachable, five of them this cause — `xsl:key/@name`, `xsl:key/@match`,
`xsl:attribute-set/@name`, `xsl:call-template/@name` and `xsl:variable/@name`. All five now raise
XTSE0010 and name the attribute. The remaining dereferences are deliberately untouched: no test
reaches them, and several of those attributes are optional in context, so a blind sweep would risk
rejecting valid stylesheets.

### The conformance figures this project published were overstated

Not an engine change. A defect in our own test harness, described here because the numbers went
out under our name.

Tests that expect a specific error code were scored as passes whenever the transform threw
**anything at all**. The W3C corpora write the expected code as an attribute —
`<error code="XTSE0010"/>` — and both conformance runners read it from the element's text content,
which is empty for an empty element. The comparison was therefore always against `""`, and empty
matched everything. `fn:load-xquery-module`, which this engine does not implement, scored 4/4 on
the strength of throwing four times.

Fixing it cost 4.6 points. Reading the code from the right place gave most of that back — many
errors carry their code in a structured property rather than in the message text, so a
message-only comparison had been under-crediting correct behaviour just as hard as it
over-credited wrong behaviour.

| | published 2026-09-02 | corrected |
|---|---|---|
| W3C XSLT 3.0 | 10,224/10,630 (96.2%) | **10,034/10,630 (94.4%)** |
| W3C XQTS (XQuery) | 95.11% | **29,205/31,414 (92.97%)** |

Every XSLT conformance figure published before 2026-09-04 is overstated by roughly two points.
Historical entries in this file are left as written, with a correction banner at the top — they
record what was believed at the time. Runs now also report how many failures are "raised an error
with the wrong code", because that is a different and generally shallower defect than a wrong
result, and conflating them is what hid this in the first place.

### Casting and constructor functions leaked .NET exceptions

`xs:int('abc')` reported *"The input string 'abc' was not in a correct format."* — a raw
`FormatException`, not an XQuery error, and unmatchable against any expected code. The same
applied to numeric overflow and to casting from a type with no conversion at all.

A constructor function is defined by the spec as equivalent to a cast, so the two must report the
same codes; only the cast half had been wrapped. Both now translate: an invalid lexical form is
FORG0001, a value out of range is FOCA0002, an operand type with no conversion is XPTY0004.

Also fixed: casting to a gregorian type from an operand the spec does not permit — `xs:time` or a
different gregorian type — diagnosed a malformed lexical form for a cast that was never legal;
`fn:avg` reported XPTY0004 for boolean and anyURI operands where `fn:sum`, three lines away in the
same file, had always correctly reported FORG0006; and `fn:load-xquery-module` flattened every
failure to FOQM0002, discarding the analyzer's own classification.

Arrives via PhoenixmlDb.XQuery, so it reaches XSLT through the engine dependency.

### Source layout: one type per file

No behaviour change, and worth stating plainly because the diff is large. 37 files carrying five
or more top-level types are now one type per file, and the two files that resisted that —
`DefaultXsltExecutionContext` at 24,638 lines and `StylesheetParser` at 13,009 — are split into
partial classes grouped by concern rather than chopped by size. Largest file in either is now
about 5,000 lines.

The split was done with Roslyn rather than text matching, deliberately: this is an XSLT engine, so
string literals are full of AVT braces and brace-counting mis-locates member boundaries. It was
verified by comparing the member inventory before and after — 587 members, none missing, none
added, each matched on whitespace-normalised text — and by confirming that every field kept its
original relative order, which is the one way a partial split can change semantics.

One deliberate exception: a `file`-scoped helper became `internal`, because a file-scoped type
cannot survive being split away from its callers. It remains assembly-internal; no public surface
changed.
...
- **A document-level `copy-of(.)` / `snapshot(.)` in a streamable body now copies the whole streamed input instead of vanishing.** At the top of an `xsl:source-document streamable="yes"` body (or a `match="/"` document-node template) the context item is the document node, so `copy-of(.)` / `snapshot(.)` copies the entire input. There is no per-node streaming dispatch for a whole-document copy, so the select evaluated against the synthetic empty document node and produced nothing — a `<head/> copy-of(.) <tail/>` body emitted only the grounded `<head/><tail/>`, dropping the copied document. The document-level whole-input-buffer detector now recognizes the bare-context-item `snapshot`/`copy-of` shape (peeling a wrapping single-item sequence / filter) and buffers the input so the copy runs against the real document root. The match is deliberately narrow — only the bare context item qualifies; a `copy-of(downward/path)` stays on the streaming Snapshot-watcher path. Closes `strm/sf-copy-of` sf-copy-of-011 and `strm/sf-snapshot` sf-snapshot-0311. (The sibling typed-attribute cases sf-copy-of-003 / sf-snapshot-0303 — `copy-of(.../@value)` bound into an `attribute(*)*` variable — remain a separate open item: they need attribute-node materialization, not the whole-input-buffer routing.)
...
- **Internal: `xsl:element`, literal result elements, and the `xsl:copy` element shell now construct temporary-tree nodes directly in the XDM node model** (via the in-memory tree builder) rather than serializing to XML text and re-parsing, for temp-tree bodies built entirely from those instructions; bodies that still contain a source-node copy (`xsl:copy-of` of existing nodes, built-in copies) continue to use the serialize-reparse path. The migrated path is proven byte-identical to the old one (differential dual-run over the full non-streaming corpus); no user-visible effect in this release. This is the groundwork that will let `inherit-namespaces="no"` prefixed-namespace suppression and no-namespace-under-default-namespace (`xmlns=""`) be corrected at the node-model source once the source-node copy path is migrated.
- **`inherit-namespaces="no"` and `copy-namespaces="no"` now produce the correct in-scope namespaces on copied content, and the base URI of a copied element built into a temporary tree is preserved correctly.** `xsl:copy`/`xsl:copy-of` of source nodes into a temporary tree now build the copy directly in the XDM node model — where namespace undeclaration is representable and per-element base URI is node data — instead of serializing to XML text and re-parsing (XML 1.0 text cannot express a prefixed namespace undeclaration, so an inherited prefix leaked onto copied descendants and a no-namespace copy under a default-namespace parent missed its `xmlns=""`). A copied element now carries exactly its own copy-namespaces-filtered in-scope set and does not acquire the constructing element's namespaces, so `in-scope-prefixes()`/`namespace-uri-for-prefix()` on the copy are correct — bringing the W3C `insn/copy` set to 119/128 (copy-0613, -0615, -0621, -0623, -1221) and also correcting `fn/base-uri` for a copied element attached to a constructed document node. The change is scoped to source-node copies built into a temporary tree; unrelated copy and construction paths are unaffected. (A separate node-model correctness fix along the way: `string()`/`xsl:value-of` on an element copied via `xsl:copy-of` into a temporary tree previously returned the empty string; it now returns the element's string value.)

### Simple content construction (`xsl:variable` temporary trees)

- **A temporary tree (an `xsl:variable` with a body and no `as=`) built from adjacent atomic values now separates them with a single space, per XSLT 3.0 §5.7.1/§5.7.2.** A variable whose body was two `xsl:sequence select="''"` collapsed to the empty string instead of a single space, because the body routed each atomic through a fresh sequence-accumulator and the accumulator drain serialized the items without applying the "single space between adjacent atomic values" rule (the plain-string serialization path treats strings as non-atomic text). The drain now runs through the shared sequence-serializer, so `('a','b')` in a temp-tree body yields `a b` and two empty strings yield a single space. This space then correctly survives into `xsl:comment`, `xsl:processing-instruction`, `xsl:attribute`, and `xsl:value-of` simple content where the temp tree is inserted via `xsl:copy-of` (document nodes stay distinct, space-separated items) — bringing the W3C `misc/seqtor` set to 71/72 (`seqtor-036a`, `-037a`, `-039a`, `-040a`). The change is scoped to the no-`as` variable-body drain; attribute-content (no separator) and already-serialized (element/comment/PI) paths are unaffected.
...
- **`xsl:number` now reuses the last `start-at` value when the value sequence is longer than the `start-at` list, and produces correct German (`lang="de"`) cardinal and ordinal number words.** Four defects are fixed, bringing the W3C `insn/number` set to 265/267. (1) With `value=` a sequence and a whitespace-separated `start-at` list, items beyond the last `start-at` value were left un-offset (defaulted to 1) instead of reusing the final `start-at` value as XSLT 3.0 requires (`number-0107`, `-0108`). (2) German cardinals rendered 1000 as "tausend" rather than "eintausend", so `format="W"` yielded `TAUSEND`/`TAUSEND FÜNF` instead of `EINTAUSEND`/`EINTAUSENDFÜNF`; values below a million are now composed as a single word (no space around "tausend"). (3) Upper-casing German number words (`format="W"`) left ß unchanged (`DREIßIG`); it now maps to the orthographically correct `SS` (`DREISSIG`). (4) German ordinals appended a naive "ste" to the whole cardinal (`dreiste`, `zehnste`); they now ordinalize only the trailing component with correct stems — including the irregulars erst/dritt/siebt/acht and the "-t" (1–19) vs "-st" (20+, round hundreds/thousands) rule — and honour the `ordinal` inflection ending (`-e`/`-er`/`-es`/`-en`, or the CLDR `%spellout-ordinal` default `-e`), yielding `dritte`, `zehnter`, `dreizehntes`, `zwanzigsten`, `zweihunderterste`, `EINTAUSENDSTEN` (`number-0812`, `-0813`, `-0828`). Italian word numbering (`number-0829`) is unaffected — the engine has no Italian data and still falls back to English.

### Grouping (`xsl:for-each-group`)

- **Composite grouping keys are now exposed as a tuple, per-item key sequences join each group once, and dateTime grouping keys compare by their instant — bringing the W3C `insn/for-each-group` set to 81/81.** Three defects are fixed. (1) With `composite="yes"`, `current-grouping-key()` returned the key sequence as a single space-joined value, so `current-grouping-key()[1]`/`[2]` and `count(current-grouping-key())` were wrong (`italy 5`/count 1 instead of `italy`,`5`/count 2); the composite key is now unwrapped to a real sequence (`for-each-group-043`, `-072`, `-087`). (2) When a non-composite `group-by` expression yielded a key *sequence* containing duplicate values for one item (e.g. `number(@pop), string-length(@name)` = `(5, 5)`), that item was added to the target group once per duplicate value; it now joins each distinct group exactly once (`for-each-group-033`). (3) `xs:dateTime` grouping keys were bucketed by their lexical form (including the timezone offset), so equal instants written in different timezones (`13:00+02:00` vs `11:00Z`) landed in different groups; grouping-key comparison now normalizes dateTime keys to their instant with the implicit timezone applied to timezone-less values, while keeping a dateTime key distinct (not an error) from a string/numeric key of the same lexical form (`for-each-group-061`, `-062`, `-064`, `-065`). The changes are scoped to the `xsl:for-each-group` grouping-key path; no cross-package (Core/XQuery) atomic-comparison changes were required.
...
- **An `as`-typed `xsl:variable`/`xsl:param` whose body is an `xsl:sequence` with content now preserves construction (document) order when interleaving literal-result-elements with `xsl:sequence select=` results.** When a typed variable body produced both literal-result-elements (which stream to the output buffer) and `xsl:sequence select=` atomic values (which go to the sequence accumulator), the two were reassembled as two segregated buckets — every accumulator item first, then every serialized node — so a body such as `<e>5</e><xsl:sequence select=".*10"/><f>6</f><xsl:sequence select=".*10+1"/><g>7</g>` iterated over `1 to 3` under `as="xs:integer*"` produced `10 11 20 21 30 31 5 6 7 5 6 7 5 6 7` instead of the correct `5 10 6 11 7 5 20 6 21 7 5 30 6 31 7`. The variable-binding path now installs the same `AsBodyCapture` position-tracking the `as`-typed template/function body path already used, recording the output offset at which each accumulator item was produced, and weaves the accumulator items and parsed serialized chunks back together in that order. The `as="document-node()"` case (a single node wrapping the whole body) keeps its historical whole-body ordering (`insn/sequence/sequence-0137a`).

### Serialization

- **The default output-method resolution now matches an `html` document element case-sensitively (lowercase).** When `xsl:output` declares no explicit `method`, a non-streamed result whose document element is `html` is promoted to the `html`/`xhtml` output method — which injects the Content-Type `<meta>`, the HTML5 DOCTYPE, and the HTML indentation default. That promotion was matching the document-element local name case-insensitively, so an **uppercase** `<HTML>` root (e.g. an identity/copy transform of an uppercase-HTML source) was wrongly promoted to the `html` method and had a `<meta http-equiv="Content-Type">` injected into `<HEAD>`, corrupting the result tree. The default-method selection now matches only a lowercase `html` local name: a lowercase `<html>` root still defaults to the html method with the injected meta (`decl/output` output-0715/0130), while an uppercase `<HTML>` root keeps the `xml` default and gets no injected meta (`insn/sequence` sequence-0601). This affects only the default-method *selection*; once a method is chosen, the html/xhtml method's own element handling (void elements, script/style raw text, the HTML5 DOCTYPE name) remains case-insensitive as the serialization spec requires.
...
- **The HTML output method no longer injects insignificant whitespace into a block element that contains character data.** The HTML indenter inserted a newline + indent before the closing tag of every block element, so an element holding text — `<body>hello</body>`, `<h1>Section 1 of 3</h1>`, `<p>…</p>` — was split across lines (`<body>hello`⏎`</body>`). That both broke serialization-string matches and added insignificant whitespace to the element's string value, so an `assert-xml` tree comparison over secondary result documents failed. A block element whose closing tag is immediately preceded by character data now serializes inline; a closing tag preceded by a child-element boundary still indents normally. Applies to all `html`-method output, principal and `xsl:result-document` alike (`insn/result-document/result-document-0701`, `-1301`).
- **SEPM0009 is no longer raised for the `text`, `html`, `json`, or `adaptive` output methods.** The `omit-xml-declaration` / `standalone` conflict check (SEPM0009) fired regardless of output method, so a named `xsl:output method="text"` that carried `omit-xml-declaration="yes"` together with a `standalone` value (e.g. one surviving an import-precedence override of another `xsl:output` of the same name) aborted with SEPM0009 instead of serializing as text. Those two parameters apply only to the `xml` and `xhtml` methods (which emit an XML declaration); the check is now gated to those methods (`insn/result-document/result-document-0239`).
...
- **`xsl:result-document` now honours its own `parameter-document` serialization attribute.** The attribute was never parsed on `xsl:result-document`, so a result-document that took its serialization parameters from an external `output:serialization-parameters` document was serialized with the wrong method and no character maps — a `method="json"` parameter document over an `xsl:map` produced adaptive-style output with unquoted string values and an errant `<?xml?>` declaration (`{"a":AAA,…}`) instead of JSON. Because the `parameter-document` URI is an attribute value template (its href may reference variables — `result-document-1406` uses `parameter-document="result-document-{$o}-params.xml"`), it is now resolved and loaded at run time (resolved against the module base URI, gated by the resource policy, XTDE0010 on read/well-formedness failure). Its output `method` and inline `output:use-character-maps` supplement the result-document (parameters set directly on `xsl:result-document` take precedence per §27.1); the character maps flow through the shared `FinalizeOutput` character-map pipeline for both the principal and secondary (href) result-document paths — so the JSON map serializes with quoted string values, no XML declaration, and its keys rewritten by the character map (`{"AAA":"AAA",…}`) (`insn/result-document/result-document-1406`). Other simple serialization parameters carried by a result-document parameter document beyond `method` and character maps are not yet consumed.
